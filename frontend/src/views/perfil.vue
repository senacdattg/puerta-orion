      ...
      <div class="perfil-content" :class="{ 'is-loading': isLoading }">
        ...
        <div v-else class="grid">
          ...
          <div class="perfil-grid" v-if="detalle && detalle.persona">
            ...
            <!-- Acudiente -->
            <div class="perfil-card" v-if="detalle?.informacion_acudiente">
              <div class="card-header">
                <h3>👨‍👩‍👧 Información de Acudiente</h3>
              </div>
              <div class="card-content">
                <div class="info-row"><label>Es responsable:</label>
                  <span>
                    <span class="badge" :class="detalle.informacion_acudiente.es_respondable ? 'badge-success' : 'badge-muted'">
                      {{ detalle.informacion_acudiente.es_respondable ? 'Sí' : 'No' }}
                    </span>
                  </span>
                </div>
              </div>
...
async function cargarCatalogosPerfil() {
  try {
    const endpoints = [
      ...
      `${baseURL}/api/catalogos/grupos-sanguineos`,
      `${baseURL}/api/catalogos/ciudades-residencia`,
      `${baseURL}/api/catalogos/eps`,
      `${baseURL}/api/catalogos/tipos-documento`,
      `${baseURL}/api/catalogos/sexos`,
      `${baseURL}/api/catalogos/deportes`,
      `${baseURL}/api/catalogos/escuelas`,
      `${baseURL}/api/catalogos/instituciones-registro`,
      `${baseURL}/api/catalogos/tipos-enfermedad`,
      `${baseURL}/api/catalogos/diagnosticos`
    ]
